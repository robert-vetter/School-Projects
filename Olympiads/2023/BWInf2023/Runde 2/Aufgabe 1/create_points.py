...
def create_points_with_right_angles(num_points, scale=500):
    ...
    for _ in range(num_points):
        ...
        step = random.uniform(0.1, 1) * scale
    ...
    while not valid:
        ...
        if decision == 1:
            ...
            while not complete_route:
                ...
                end_time = time.time()
            print("Die Länge der erstellten Route beträgt ", route_length(coordinates), " km.")
            ...
            absolute_path = str(input("Geben Sie den absoluten Pfad der txt-Datei mit den Koordinaten an (Bsp.: X:\wenigerkrumm1.txt): "))
            coordinates = read_coordinates(absolute_path)
            start_time = time.time()
            complete_route = find_complete_route(coordinates)
            print("Die Länge der mit dem Algorithmus berechneten Route beträgt ", route_length(complete_route), " km.")
            end_time = time.time()
