        ...
        while unvisited:
            ...
            for i in unvisited:
                ...
                if len(route) > 1:
                    ...
                    angle = 100
                if angle >= 90:
                    temp_distance = calculate_distance(route[-1], point)  # Verwenden Sie die calculate_distance-Funktion
            ...
            while len(remaining_points(coordinates, new_route)) > 0:
                ...
                if new_route is None:
                    ...
                    for route in new_route:
                        shortest_route =  min(new_route, key=lambda route: route_length(route))
                        print("Die Länge der Route beträgt ", route_length(shortest_route), " km.")
                        end_time = time.time()
                        print("Dauer: ", end_time - start_time, " Sekunden")
                        plot_route(np.array(coordinates), shortest_route)
